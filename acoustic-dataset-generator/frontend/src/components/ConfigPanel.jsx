  ...
  return (
    <div className="field">
      <div className="field-header">
        <label htmlFor={name} style={{ display: "flex", alignItems: "center", gap: 8 }}>
...
export default function ConfigPanel({
  ...
  onToggleSweep = null,
  sweepOpen = false,
  onToggleSweepOpen = null,
}) {
  const [activeTab, setActiveTab] = useState("dataset");
  const isRunning = status === "running";

  const mappedToggleSweep = useMemo(() => {
    // Sweep artık sabit panelden yönetiliyor
    return null;
  }, []);

  function ensureSources(count) {
    const n = Math.max(1, Math.min(8, Number(count) || 1));
    const cur = Array.isArray(config.sources) ? config.sources : [];
    const next = cur.slice(0, n);
    while (next.length < n) {
      next.push({ signal_type: "white_noise", custom_audio_file: null });
    }
    return { n, sources: next };
  }
  ...
  function set(name, value) {
    ...
    if (name === "source_count") {
      const { n, sources } = ensureSources(value);
      updated = { ...updated, source_count: n, sources };
    }

    // Oda boyutu değişince (veya mic_radius değişince) far-field seçiliyse min/max'i yeni oda sınırına göre güncelle
    if (updated.field_mode === "farfield" && (name === "room_x" || name === "room_y" || name === "room_z" || name === "mic_radius" || name === "mic_center_x" || name === "mic_center_y")) {
      ...
      // Far-field aralığı: [ffBoundary, odaMax]
      // Eğer ffBoundary odaMax'i aşıyorsa, far-field fiziksel olarak mümkün değil; setFieldMode zaten engelliyor.
      // Burada yine de güvenli clamp yapalım.
      const newMin = Math.min(newFF, newAbsMax);
      const newMax = newAbsMax;
      updated = {
        ...updated,
        min_distance: parseFloat(newMin.toFixed(2)),
        max_distance: parseFloat(newMax.toFixed(2)),
      };
    }
  ...
  const sourceCount = Math.max(1, Math.min(8, Number(config.source_count) || 1));
  const isMulti = sourceCount > 1;
  const canStart = isMulti
    ? (config.sources || []).slice(0, sourceCount).every((s) => s.signal_type !== "custom" || !!s.custom_audio_file)
    : ((config.sources?.[0]?.signal_type !== "custom") || !!config.sources?.[0]?.custom_audio_file);
    ...
    <aside className="config-panel">
      ...
      {activeTab === "dataset" && (
        <div className="tab-content">
          ...
          <div className="section-divider">Kaynaklar</div>
          ...
          {isMulti && (
            <>
              {(config.sources || []).slice(0, sourceCount).map((src, idx) => (
                <div key={idx} style={{ marginTop: 10, paddingTop: 8, borderTop: "1px solid rgba(255,255,255,0.08)" }}>
                  <div className="field-header" style={{ marginBottom: 6 }}>
                    <label>Kaynak #{idx + 1}</label>
                  </div>

                  <div className="field">
                    <div className="field-header"><label>Signal Type</label></div>
                    <select
                      value={src.signal_type || "white_noise"}
                      disabled={isRunning}
                      onChange={(e) => {
                        const nextSources = [...(config.sources || [])];
                        nextSources[idx] = { ...nextSources[idx], signal_type: e.target.value, custom_audio_file: null };
                  ...
                  {src.signal_type === "custom" && (
                    <AudioUpload
                      selectedFile={src.custom_audio_file || null}
                      onSelect={(f) => {
                        const nextSources = [...(config.sources || [])];
                        nextSources[idx] = { ...nextSources[idx], custom_audio_file: f };
                        onChange({ ...config, sources: nextSources });
                      }}
                    />
                  )}

                  {src.signal_type === "custom" && !src.custom_audio_file && (
                    <div className="warn-box">⚠ Kaynak #{idx + 1} için bir ses dosyası seçin veya yükleyin</div>
                  )}
                </div>
              ))}
            </>
          ...
          {!isMulti && (
            <>
              ...
              <div className="field">
                ...
                <select
                  value={config.sources?.[0]?.signal_type || "white_noise"}
                  disabled={isRunning}
                  onChange={(e) => {
                    const nextSources = [...(config.sources || [])];
                    nextSources[0] = { ...(nextSources[0] || {}), signal_type: e.target.value, custom_audio_file: null };
                    onChange({ ...config, sources: nextSources });
                  }}
                >
                  {SIGNAL_TYPES.map((t) => (
              ...
              {config.sources?.[0]?.signal_type === "custom" && (
                <>
                  <AudioUpload
                    selectedFile={config.sources?.[0]?.custom_audio_file || null}
                    onSelect={(f) => {
                      const nextSources = [...(config.sources || [])];
                      nextSources[0] = { ...(nextSources[0] || {}), custom_audio_file: f };
                      onChange({ ...config, sources: nextSources });
                    }}
                  />
                </>
      ...
      {activeTab === "mic" && (
        <div className="tab-content">
          ...
          <label className={`radio-row ${config.center_mic ? "on" : ""}`}>
            <input
              type="checkbox"
          ...
          <button
            className="mode-btn compact-btn"
      ...
      {activeTab === "env" && (
        <div className="tab-content">
              ...
              <small>
                ...
                {!farFieldPossible && (
                  <span style={{ color: "#f87171" }}>
                    {" "}⚠ ff sınırı odadan büyük, far-field devre dışı
                  </span>
                )}
              </small>
            </div>
          ...
          <SliderField label="Ortam SNR (düşük = daha gürültülü)" name="ambient_snr_db"
            ...
            value={config.ambient_snr_db} onChange={set} unit=" dB"
          />

          <AudioUpload
            kind="ambient"
            selectedFile={config.ambient_audio_file || null}
            onSelect={(f) => onChange({ ...config, ambient_audio_file: f })}
          />

          {config.ambient_audio_file && (
            <div className="info-box">
              Ambient dosya: <code>{config.ambient_audio_file}</code>
            </div>
          )}
        </div>
      )}
      ...
      {activeTab === "output" && (
        <div className="tab-content">
          <div className="section-divider">Sweep</div>

          <button
            type="button"
            className={`mode-btn sweep-open-btn ${sweepOpen ? "active" : ""}`}
            onClick={() => (onToggleSweepOpen ? onToggleSweepOpen() : null)}
            disabled={isRunning}
            title={isRunning ? "Çalışırken sweep paneli açılamaz" : "Sweep panelini aç/kapat"}
          >
            {sweepOpen ? "Sweep Panelini Kapat" : "Sweep Panelini Aç"}
          </button>

          <div className="info-box">
            Sweep parametreleri sabittir: kaynak dosya grupları, SNR, oda boyutu, RT60, ambient SNR ve ambient dosya.
          </div>
