...
export default function PolarView({ label, nMics, micRadius, centerMic,
  ...
  // Kaynak pozisyonu: dünya koordinatında, orijin = MCX,MCY
  const srcW = label
    ? { wx: label.source_pos[0] - MCX, wy: label.source_pos[1] - MCY }
    : null;
  ...
  return (
    <div className="polar-view-fullscreen" ref={containerRef}>
      ...
      <div className="polar-overlay-bar">
        ...
        {label && (
          <span className="polar-overlay-info">
            {label.azimuth_deg.toFixed(1)}°
            &nbsp;/&nbsp;
            {label.distance_m.toFixed(2)} m
            &nbsp;·&nbsp;
            ({label.source_pos[0].toFixed(2)}, {label.source_pos[1].toFixed(2)})
          </span>
        )}
      </div>
      ...
      >
        ...
        {srcW && (() => {
          const [sx, sy] = w2s(srcW.wx, srcW.wy);
          return (
            <>
              <line x1={ox} y1={oy} x2={sx} y2={sy}
                stroke="#f97316" strokeWidth={1.5} strokeDasharray="5,3" opacity={0.7} />
              <circle cx={sx} cy={sy} r={10} fill="#f9731620" stroke="#f97316" strokeWidth={1.5} />
              <circle cx={sx} cy={sy} r={4} fill="#f97316" />
              <text x={sx + 10} y={sy - 8} fill="#f97316" fontSize={FS} fontFamily="monospace">
                {label.azimuth_deg.toFixed(1)}° / {label.distance_m.toFixed(2)}m
              </text>
            </>
          );
        })()}
